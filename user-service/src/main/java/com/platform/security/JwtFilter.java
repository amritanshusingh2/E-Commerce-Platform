...
public class JwtFilter extends GenericFilter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        logger.debug("[JwtFilter] Processing request: {} {}", httpRequest.getMethod(), httpRequest.getRequestURI());
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            try {
                ...
                logger.debug("[JwtFilter] Extracted username: {}", username);
                ...
                if (username != null && jwtUtil.validateToken(jwtToken)) {
                    try {
                        ...
                        logger.debug("[JwtFilter] Authentication set successfully for user: {} with authorities: {}", 
                                   username, userDetails.getAuthorities());
                    } catch (Exception e) {
                        logger.error("[JwtFilter] Error loading user details for username: {}", username, e);
                        // Fallback: create authentication from JWT claims directly
                        ...
                        if (rolesObj != null) {
                            ...
                            SecurityContextHolder.getContext().setAuthentication(auth);
                            logger.debug("[JwtFilter] Fallback authentication set for user: {} with authorities: {}", 
